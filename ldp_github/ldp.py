...
by Jacqueline Maasch, Weishen Pan, Shantanu Gupta, Volodymyr Kuleshov, Kyra Gan, Fei Wang.
Original code can be found at https://github.com/jmaasch/ldp
"""
...
class LDP:
    ...
    def __init__(self, data: pd.DataFrame, indep_test):
    ...
    def ind_test(self, var_0: str, var_1: str, conditioning_set: list = None) -> float:
        """
    ...
    def oracle(self, var_0, var_1, conditioning_set=None) -> float:
        """
        ...
        if self.dag is None:
            raise ValueError(
                "self.dag is None; must supply ground truth DAG as numpy array to use oracle."
            )
        if self.var_names is None:
            raise ValueError(
                "self.var_names is None; must supply ground truth variable names as list to use oracle."
            )
    ...
    def partition_z(
        ...
        start = time.time()
        self.partition_z_steps_1_2_3(
            exposure=exposure, outcome=outcome, alpha=alpha, verbose=verbose
        )
        if verbose:
            print(
                "** Steps 1–3 complete in {} seconds.".format(
        ...
        if len(self.z4) > 0:
            ...
            if verbose:
                print(
                    "** Step 4 complete in {} seconds.".format(
                        round(time.time() - start, 4)
                    )
                )
        else:
            if verbose:
                print(
                    "!! WARNING: Condition 2 unsatisfied (no Z4 discovered). Z_post that are descended from Y might be unidentifiable. A valid adjustment set might still be identifiable."
                )
        ...
        if alpha_step_5 is None:
            self.partition_z_step_5(
                exposure=exposure, outcome=outcome, alpha=alpha, verbose=verbose
            )
        else:
            self.partition_z_step_5(
                exposure=exposure, outcome=outcome, alpha=alpha_step_5, verbose=verbose
            )
        if verbose:
            print(
                "** Step 5 complete in {} seconds.".format(
                    round(time.time() - start, 4)
                )
            )
        ...
        if verbose:
            print(
                "** Step 6 complete in {} seconds.".format(
                    round(time.time() - start, 4)
                )
            )
        ...
        start = time.time()
        self.partition_z_step_7(
            exposure=exposure, outcome=outcome, alpha=alpha, verbose=verbose
        )

        if verbose:
            print(
                "** Step 7 complete in {} seconds.".format(
                    round(time.time() - start, 4)
                )
            )
        ...
        start = time.time()
        self.test_z5_criterion(
            exposure=exposure, outcome=outcome, alpha=alpha, verbose=verbose
        )

        if verbose:
            print(
                "** Step 8 complete in {} seconds.".format(
                    round(time.time() - start, 4)
                )
            )
        ...
        results = {
            "Predicted label": list(self.pred_label_dict.values()),
            ...
            "Z8": self.z8,
            "Z_post": self.z_post,
            "vas_exists": self.vas_exists,
        }
        return results

    def partition_z_steps_1_2_3(
        self,
        exposure: str = "X",
        outcome: str = "Y",
        alpha: float = 0.005,
    ...
    def partition_z_step_5(
        ...
        for candidate in self.z_prime:
            ...
            is_parent_of_x = self.test_z5_z7(
                conditioning_set=conditioning_set,
                exposure=exposure,
                outcome=outcome,
                candidate=candidate,
        ...
        if len(self.z_prime) > 0:
            ...
            for candidate in self.z_prime:
                if len(z_mix) == 0:
                    if verbose:
                        print(
                            "!! WARNING: Condition 3 unsatisfied (no potential Z1_Z5 discovered). Z1/Z3 not identifiable."
                        )
                else:
                    ...
                    for var in z_mix:
                        ...
                        m_ind = self.ind_test(
                            var_0=var, var_1=candidate, conditioning_set=None
                        )
                        ind_results[var] = m_ind > alpha

                        # Test conditional independence given the exposure.
                        c_ind = self.ind_test(
                            var_0=var, var_1=candidate, conditioning_set=[exposure]
                        )
                        ...
                        if m_ind > alpha and c_ind <= alpha:
                            ...
                            if verbose:
                                print(
                                    "{} -> X <- {}: {} is a confounder (STEP 6.1).".format(
                    ...
                    if not identified_confounder:
                        ...
                        self.z_post.append(candidate)
                        self.pred_label_dict[candidate] = (
                            "Z2 or Z3 or Z6"  # Previously, just "Z3"
                        )
            ...
            for z_mix in z_mix_set:
                for z1_z5 in self.z1_z5:
                    ...
                    m_ind = self.ind_test(
                        var_0=z_mix, var_1=z1_z5, conditioning_set=None
                    )
                    # Test conditional independence given the exposure.
                    ...
                    if m_ind > alpha:
                        if verbose:
                            print(
                                "{} -> X <- {}: {} is a confounder (STEP 6.2).".format(
                                    z1_z5, z_mix, z_mix
                                )
                            )
    ...
    def partition_z_step_7(
        ...
        if len(self.z1_z5) == 0 or len(self.z1) == 0:
            if verbose:
                print(
                    "!! WARNING: Cannot perform Step 7: len(self.z1_z5) == 0 or len(self.z1) == 0. Consequently, a valid adjustment set could not be identified."
                )
            return
        else:
                ...
                for confounder in previously_known_confounders:
                    ...
                    if ind_results is None:
                        ...
                        if not ind:
                            if verbose:
                                print(
                                    "{} vs {}: {} is in Z1, remove from Z5.".format(
                                        candidate, confounder, candidate
                                    )
                                )
                            self.z1.append(candidate)
                            self.pred_label_dict[candidate] = "Z1"
    ...
    def test_z5_criterion(
        ...
        for z5 in self.z5:
            ind = self.ind_test(
                var_0=z5, var_1=outcome, conditioning_set=self.z1 + [exposure]
            )
            if ind > alpha:
        ...
        if not self.vas_exists and verbose:
            print(
                "!! WARNING: A valid adjustment set does not exist in Z, or it is unidentifiable (e.g., due to assumption violations)."
            )
            return
    ...
    def test_z8(
        ...
        if candidate in self.x_ind_z_dict:
            ...
            x_ind_z = self.ind_test(
                var_0=exposure, var_1=candidate, conditioning_set=None
            )
        ...
        if candidate in self.y_ind_z_dict:
            ...
            y_ind_z = self.ind_test(
                var_0=outcome, var_1=candidate, conditioning_set=None
            )
    ...
    def test_z4(
        ...
        x_ind_z_given_y = self.ind_test(
            var_0=exposure, var_1=candidate, conditioning_set=[outcome]
        )
    ...
    def test_z5_z7(
        ...
        y_ind_z_given_x = self.ind_test(
            var_0=outcome, var_1=candidate, conditioning_set=conditioning_set
        )
    ...
    def test_zpost(
        ...
        for var in causes_outcome:
            m_ind = self.ind_test(var_0=var, var_1=candidate, conditioning_set=None)
            m_ind_given_exposure_outcome = self.ind_test(
                var_0=var, var_1=candidate, conditioning_set=[exposure, outcome]
            )
    ...
    def test_v_structure(
        ...
        m_ind = self.ind_test(var_0=causes[0], var_1=causes[1], conditioning_set=None)

        # Test conditional independence.
        c_ind = self.ind_test(
            var_0=causes[0], var_1=causes[1], conditioning_set=[collider]
        )
